Ext.define('Admin.view.sign.SignViewController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.signViewController',
    /*Add*/
//	openAddWindow:function(grid, rowIndex, colIndex){
//			grid.up('sign').add(Ext.widget('signAddWindow')).show();
//	},
//	submitAddForm:function(btn){
//		var win    = btn.up('window');
//		var form = win.down('form');
//		var record = Ext.create('Admin.model.sign.SignModel');
//		var values  =form.getValues();//获取form数据
//           	record.set(values);
//          	record.save();
//          	Ext.data.StoreManager.lookup('signGridStore').load();
//          	win.close();
//	},
	/*signIn */
    signIn:function(btn){
    	var myDate = new Date(); 
    	var timeFormat=Ext.Date.format(myDate, 'Y-m-d');
    	Ext.Ajax.request({ 
			url : '/Sign/signIn', 
			method : 'post', 
			params : { 
    	...
    	success: function(response, options) {
            ...
            if(json.success){
            	Ext.Msg.alert('签到成功', json.msg, function() {
            		btn.up('gridpanel').getStore().reload();
                });
	        }else{
	        	 Ext.Msg.alert('签到失败', json.msg);
	        }
        }
	})
	...
	signBack:function(btn){
    	...
    	success: function(response, options) {
            var json = Ext.util.JSON.decode(response.responseText);
            if(json.success){
            	Ext.Msg.alert('签退成功', json.msg, function() {
            		btn.up('gridpanel').getStore().reload();
                });
	        }else{
	        	 Ext.Msg.alert('签退失败', json.msg);
	        }
        }
	})
	...
	/*extraSignIn */
	extraSignIn:function(btn){
		var myDate = new Date(); 
    	var timeFormat=Ext.Date.format(myDate, 'Y-m-d');
    	Ext.Ajax.request({ 
			url : '/Sign/extraSignIn', 
			method : 'post', 
			params : { 
				//ids[] :selectIds
				extraStarTime :timeFormat
			},
    	success: function(response, options) {
            var json = Ext.util.JSON.decode(response.responseText);
            if(json.success){
            	Ext.Msg.alert('加班签到成功', json.msg, function() {
            		btn.up('gridpanel').getStore().reload();
                });
	        }else{
	        	 Ext.Msg.alert('加班签到失败', json.msg);
	        }
        }
	})
	...
	/*extraSignBack */
	extraSignBack:function(btn){
		var myDate = new Date(); 
    	var timeFormat=Ext.Date.format(myDate, 'Y-m-d');
    	Ext.Ajax.request({ 
			url : '/Sign/extraSignBack', 
			method : 'post', 
			params : { 
				//ids[] :selectIds
				extraEndTime :timeFormat
			},
    	success: function(response, options) {
            var json = Ext.util.JSON.decode(response.responseText);
            if(json.success){
...
//		if (record ) {
//			var win = grid.up('sign').add(Ext.widget('signEditWindow'));
//			win.show();
...
//		var win    = btn.up('window');
//		var store = Ext.data.StoreManager.lookup('signGridStore');
