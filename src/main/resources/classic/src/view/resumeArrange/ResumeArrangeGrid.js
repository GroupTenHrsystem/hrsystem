Ext.define('Admin.view.resumeArrange.resumeArrangeGrid', {
    extend: 'Ext.grid.Panel',
	xtype:'resumeArrangeGrid',
	title: '面试管理',		//需要修改
	iconCls: 'fa-arrow-circle-o-up',
	bind: '{resumeArrangeStore}',//调用组件4
	columns: [{
			xtype: 'actioncolumn',
			items: [{
				xtype: 'button',
				...
				handler: 'onClickResumeArrangeClaimButton'	//ajax  taskId
			},{
				xtype: 'button',
				...
				handler: 'onClickResumeArrangeCompleteWindowButton'	//taskDefinitionKey 动态表单
			},{
				xtype: 'button',
		...
		,{header: '一面面试官' ,dataIndex: 'firstarragne',width: 80,sortable: true}
		,{header: '一面时间' ,dataIndex: 'firstTime',width: 80,sortable: true,renderer: Ext.util.Format.dateRenderer('Y/m/d H:i:s'),hidden:true}
		,{header: '二面面试官' ,dataIndex: 'lastarragne',width: 80,sortable: true}
		,{header: '二面时间' ,dataIndex: 'lastTime',width: 80,sortable: true,renderer: Ext.util.Format.dateRenderer('Y/m/d H:i:s'),hidden:true}
	...
	dockedItems: [{
	    ...
	    items: ['-',{
			...
			listeners: {
				click: 'onClickResumeArrangeGridAddButton'
            }
        }, '-',{
            text: 'Update',
            iconCls: 'x-fa fa-pencil',
            listeners: {
				click: 'onClickResumeArrangeGridUpdateButton'
            }
        }, '-', {
            text: 'Delete',
            iconCls: 'x-fa fa-close',
			listeners: {
				click: 'onClickResumeArrangeGridDeleteButton'
            }
        }]
	}]
});
